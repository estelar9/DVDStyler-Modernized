...
#define CURSOR_FILE(fname)                                                     \
  wxFindDataFile(_T("rc") + wxString(wxFILE_SEP_PATH) + fname)
...
public:
  MenuDnDFile(MenuEditor *menuEditor) {
    ...
    dataObject->Add(m_actionDataObject =
                        new wxCustomDataObject(actionDataFormat));
  ...
  wxDragResult OnData(wxCoord x, wxCoord y, wxDragResult def) {
    ...
    if (dobj->GetReceivedFormat() == wxDF_FILENAME) {
      ...
      wxString actionStr((const char *)m_actionDataObject->GetData(),
                         wxConvISO8859_1, m_actionDataObject->GetSize());
      DVDAction action;
      action.Deserialize(actionStr);
      AddButton(x, y, action.GetTsi(), action.GetPgci(), action.IsMenu(),
                action.GetChapter());
      return def;
    }
  ...
  bool AddFile(wxCoord x, wxCoord y, wxString filename, bool first) {
    ...
    if (ext == _T("xml"))
      m_menuEditor->AddButton(filename, (int)(x / m_menuEditor->GetScaleX()),
                              (int)(y / m_menuEditor->GetScaleY()));
    else if (wxImage::FindHandler(ext, wxBITMAP_TYPE_ANY))
      m_menuEditor->AddImage(filename, (int)(x / m_menuEditor->GetScaleX()),
                             (int)(y / m_menuEditor->GetScaleY()));
      ...
      bool found = false;
      for (unsigned int tsi = 0;
           tsi < m_menuEditor->GetDVD()->GetTitlesets().GetCount(); tsi++) {
        ...
        for (unsigned int pgci = 0; pgci < ts.GetTitles().GetCount(); pgci++) {
          ...
          for (unsigned int vobi = 0; vobi < pgc->GetVobs().GetCount();
               vobi++) {
            ...
            if (vob->GetFilename() == filename) {
              ...
              int chapter = 0;
              if (vobi > 0)
                chapter = pgc->GetChapterCount(vobi) +
                          (vob->GetChapters().length() ? 0 : -1);
      ...
      if (!found) {
        ...
        if (tsi == -1)
          tsi = 0;
        if (m_menuEditor->GetTitlesetManager()->AddVideo(filename, true, tsi,
                                                         false)) {
          int pgci =
              m_menuEditor->GetDVD()->GetPgcArray(tsi, false).GetCount() - 1;
  ...
  void AddButton(wxCoord x, wxCoord y, int tsi, int pgci, bool isMenu,
                 int chapter, bool force = false) {
    wxString objId = m_menuEditor->HitTest(x, y);
    if (!force && objId.length() &&
        m_menuEditor->GetMenu()->GetObject(objId)->IsButton()) {
      ...
      if (isMenu)
        m_menuEditor->AddButton(BUTTONS_DIR + s_config.GetDefTextButton(), x, y,
                                pgci, tsi, isMenu, chapter);
      else
        m_menuEditor->AddButton(BUTTONS_DIR + s_config.GetDefButton(), x, y,
                                pgci, tsi, isMenu, chapter);
    }
  }
...
EVT_UPDATE_UI(MENUEDITOR_OBJMENU_FIRSTBUTTON_ID,
              MenuEditor::OnUpdateUIFirstButton)
...
EVT_UPDATE_UI(MENUEDITOR_ARRANGEMENU_TOFRONT_ID,
              MenuEditor::OnUpdateUIObjectForward)
EVT_UPDATE_UI(MENUEDITOR_ARRANGEMENU_FORWARD_ID,
              MenuEditor::OnUpdateUIObjectForward)
EVT_UPDATE_UI(MENUEDITOR_ARRANGEMENU_BACKWARD_ID,
              MenuEditor::OnUpdateUIObjectBackward)
EVT_UPDATE_UI(MENUEDITOR_ARRANGEMENU_TOBACK_ID,
              MenuEditor::OnUpdateUIObjectBackward)
...
EVT_MENU_RANGE(MENUEDITOR_ADDMENU_OBJECT_ID, MENUEDITOR_ADDMENU_OBJECT_ID + 999,
               MenuEditor::OnAddObject)
// PERF FIX: timer for throttled SVG refresh during drag
EVT_TIMER(EVENT_TIMER_ID, MenuEditor::OnRefreshTimer)
END_EVENT_TABLE()
...
MenuEditor::MenuEditor(wxWindow *parent, wxWindowID id)
    : wxSVGCtrl(parent, id, wxDefaultPosition, wxDefaultSize, wxWANTS_CHARS),
      m_gridColour(255, 255, 255), m_refreshTimer(this, EVENT_TIMER_ID) {
  m_dvd = NULL;
  m_menu = NULL;
  m_doAction = false;
  DoSelect();
  ...
  m_doCopy = false;
  m_copied = false;
  // PERF FIX: init HitTest throttle state
  m_lastHitTestX = -9999;
  m_lastHitTestY = -9999;
  // PERF FIX: init cursor cache state
  m_lastCursorAction = eaNONE;
  // PERF FIX: init render throttle state
  m_refreshPending = false;
  // PERF FIX: pre-load cursors once to avoid disk I/O on every mouse move
#ifdef __WXMSW__
  m_cursorMove = wxCursor(_T("wxCURSOR_MOVE"), wxBITMAP_TYPE_CUR_RESOURCE);
  m_cursorRotate = wxCursor(_T("wxCURSOR_ROTATE"), wxBITMAP_TYPE_CUR_RESOURCE);
#else
  m_cursorMove = wxCursor(wxImage(CURSOR_FILE(_T("move.cur"))));
  m_cursorRotate = wxCursor(wxImage(CURSOR_FILE(_T("rotate.cur"))));
#endif
  m_cursorResizeNWSE = wxCursor(wxCURSOR_SIZENWSE);
  m_cursorResizeNESW = wxCursor(wxCURSOR_SIZENESW);
  ...
  alignMenu->Append(MENUEDITOR_ALIGNMENU_TOCENTERH_ID,
                    _("To Center &horizontally"));
  alignMenu->Append(MENUEDITOR_ALIGNMENU_TOCENTERV_ID,
                    _("To Center &vertically"));
  alignMenu->Append(MENUEDITOR_ALIGNMENU_TOCENTERMH_ID,
                    _("To &Center of Menu horizontally"));
  alignMenu->Append(MENUEDITOR_ALIGNMENU_TOCENTERMV_ID,
                    _("To Center of &Menu vertically"));
  ...
  viewMenu->Append(MENUEDITOR_VIEWMENU_SAFETV_ID, _("&Show Safe TV area"),
                   wxEmptyString, wxITEM_CHECK);
  viewMenu->Check(MENUEDITOR_VIEWMENU_SAFETV_ID, m_safeTV);

  // sub menu "Grid"
  m_gridMenu = new wxMenu;
  m_gridMenu->Append(MENUEDITOR_GRIDMENU_ITEM_ID, _("&none"), wxEmptyString,
                     wxITEM_RADIO);
  m_gridMenu->Append(MENUEDITOR_GRIDMENU_ITEM_ID + 1, _T("4x4"), wxEmptyString,
                     wxITEM_RADIO);
  m_gridMenu->Append(MENUEDITOR_GRIDMENU_ITEM_ID + 2, _T("8x8"), wxEmptyString,
                     wxITEM_RADIO);
  m_gridMenu->Append(MENUEDITOR_GRIDMENU_ITEM_ID + 3, _T("16x16"),
                     wxEmptyString, wxITEM_RADIO);
  m_gridMenu->Append(MENUEDITOR_GRIDMENU_ITEM_ID + 4, _T("32x32"),
                     wxEmptyString, wxITEM_RADIO);
  m_gridMenu->Append(MENUEDITOR_GRIDMENU_ITEM_ID + 5, _T("64x64"),
                     wxEmptyString, wxITEM_RADIO);
  m_gridMenu->Check(MENUEDITOR_GRIDMENU_ITEM_ID + m_grid - 1, true);
  m_gridMenu->Append(MENUEDITOR_GRIDMENU_SHOW_ID, _("&Show"), wxEmptyString,
                     wxITEM_CHECK);
  m_gridMenu->Append(MENUEDITOR_GRIDMENU_OVEROBJECTS_ID, _("&Over objects"),
                     wxEmptyString, wxITEM_CHECK);
  ...
  m_objMenu->AppendCheckItem(MENUEDITOR_OBJMENU_FIRSTBUTTON_ID,
                             _("&First highlighted button"));
  m_objMenu->AppendSeparator();
  wxMenuItem *item =
      new wxMenuItem(m_objMenu, MENUEDITOR_OBJMENU_CUT_ID, _("Cu&t"));
...
bool MenuEditor::SetBackground(wxString fname) {
  ...
  if (wxImage::FindHandler(fname.AfterLast(wxT('.')).Lower(),
                           wxBITMAP_TYPE_ANY)) {
    wxImage img;
...
bool MenuEditor::AddButton(wxString fname, int x, int y, int pgci, int tsi,
                           bool isMenu, int chapter) {
  if (!m_menu || !wxFileExists(fname))
    return false;

  if (m_menu->GetAspectRatio() == ar16_9) {
    if (m_dvd->GetPgcArray(m_tsi, true).GetVideo().GetWidescreen() == wtAUTO &&
        m_menu->GetButtonsCount() >= 12) {
      wxLogError(_("Wide screen DVD menu can contain maximal 12 buttons"));
      return false;
    } else if (m_menu->GetButtonsCount() >= 18) {
      wxLogError(_("Wide screen DVD menu (nopanscan/noletterbox) can contain "
                   "maximal 18 buttons"));
      return false;
    }
  ...
  if (initParam) {
    ...
    if (initParam->type == wxT("image")) {
      ...
      if (isMenu) {
        param = wxString::Format(tsi == -1 ? _("VMGM menu %d") : _("Menu %d"),
                                 pgci + 1);
      } else
...
bool MenuEditor::AddObject(wxString fname, int x, int y) {
  ...
  if (initParam) {
    if (initParam->type == wxT("string"))
      ...
      param = wxFileSelector(_("Please choose an image to insert"), wxT(""),
                             wxGetCwd(), wxT(""),
...
wxString MenuEditor::HitTest(int x, int y) {
  ...
  bool button = true;
  wxSVGUseElement *child =
      (wxSVGUseElement *)m_doc->GetElementById(wxT("buttons"))->GetChildren();
  while (child || button) {
    if (!child) {
      if (id.length() > 0)
        break;
      // no button was found, try to find a object
      child = (wxSVGUseElement *)m_doc->GetElementById(wxT("objects"))
                  ->GetChildren();
      button = false;
      continue;
    }
    if (child->GetType() == wxSVGXML_ELEMENT_NODE &&
      ...
      wxSVGRect bbox = wxSVGRect(
          child->GetX().GetAnimVal(), child->GetY().GetAnimVal(),
          child->GetWidth().GetAnimVal(), child->GetHeight().GetAnimVal());
      if (!bbox.IsEmpty() && p.GetX() >= bbox.GetX() &&
          p.GetX() <= bbox.GetX() + bbox.GetWidth() &&
          p.GetY() >= bbox.GetY() &&
          p.GetY() <= bbox.GetY() + bbox.GetHeight()) {
        id = child->GetId();
      }
    }
...
void MenuEditor::DoSelect(wxString id, bool refresh, bool sendEvent,
  ...
  if (!ctrlDown) {
    wxRect rect;
    for (wxArrayString::const_iterator it = m_selectedIds.begin();
         it != m_selectedIds.end(); it++) {
      wxSVGRectElement *selRect =
          (wxSVGRectElement *)m_doc->GetElementById(wxT("selection_") + *it);
      if (selRect) {
        if (refresh)
          rect.Union(ScaleRect(selRect->GetBBox(wxSVG_COORDINATES_VIEWPORT),
                               GetScaleX(), GetScaleY())
                         .Inflate(8));
  ...
  if (m_selectedIds.Index(id) != wxNOT_FOUND) {
    wxSVGRectElement *selRect =
        (wxSVGRectElement *)m_doc->GetElementById(wxT("selection_") + id);
    if (selRect) {
      if (refresh)
        RefreshRect(ScaleRect(selRect->GetBBox(wxSVG_COORDINATES_VIEWPORT),
                              GetScaleX(), GetScaleY())
                        .Inflate(8));
      selRect->GetParent()->RemoveChild(selRect);
    }
...
wxRect MenuEditor::UpdateSelectionRect(int x, int y) {
  ...
  wxRect refreshRect;
  wxSVGRectElement *selRect =
      (wxSVGRectElement *)m_doc->GetRootElement()->GetElementById(
          wxT("menuObjSelection"));
  if (selRect == NULL) {
  ...
  return refreshRect.Union(ScaleRect(
      selRect->GetBBox(wxSVG_COORDINATES_VIEWPORT), GetScaleX(), GetScaleY()));
}

wxRect MenuEditor::PerformSelection() {
  wxSVGRectElement *selRectElem =
      (wxSVGRectElement *)m_doc->GetRootElement()->GetElementById(
          wxT("menuObjSelection"));
  if (selRectElem == NULL) {
  ...
  wxRect selRect = wxRect((int)bbox.GetX(), (int)bbox.GetY(),
  ...
  for (unsigned int i = 0; i < m_menu->GetObjectsCount(); i++) {
    ...
    if (obj->GetBBox().Intersects(selRect) &&
        m_selectedIds.Index(id) == wxNOT_FOUND) {
      m_selectedIds.push_back(id);
...
wxRect MenuEditor::CreateSelection(wxString id) {
  ...
  if (obj->GetAngle() != 0 && obj->GetParamByName(wxT("rotation")) == NULL) {
    ...
    selRect->Rotate(obj->GetAngle(), bbox.GetX() + bbox.GetWidth() / 2,
...
void MenuEditor::RefreshSelected() {
  wxRect rect;
  for (wxArrayString::iterator it = m_selectedIds.begin();
       it != m_selectedIds.end(); it++)
...
void MenuEditor::OnMouseMove(wxMouseEvent &event) {
  ...
  // left mouse button is NOT down => hover mode
  if (!event.LeftIsDown()) {
    m_action = GetEditorAction(x, y);
    // change mouse cursor
    SetMouseCursor(m_action);
    // PERF FIX: throttle HitTest — only re-run if mouse moved > 4px
    if (abs(x - m_lastHitTestX) > 4 || abs(y - m_lastHitTestY) > 4) {
      m_lastHitTestX = x;
      m_lastHitTestY = y;
      m_lastHitTestResult = HitTest(x, y);
    }
    wxString pointed = m_lastHitTestResult;
  ...
  if (m_action == eaMOVE || m_action == eaSELECT) { // move Object
    ...
    int y1 = y - m_oldy;
    if (MoveObject(x1, y1)) {
      // PERF FIX: throttle SVG redraws to ~60fps during drag
      if (!m_refreshPending) {
        m_refreshPending = true;
        m_refreshTimer.Start(16, wxTIMER_ONE_SHOT);
      }
      m_doAction = true;
    }
    m_oldx += x1;
  ...
  } else if (m_action == eaNONE ||
             m_action == eaSELECT_RECT) { // rectangle selection
...
EditorAction MenuEditor::GetEditorAction(int x, int y) {
  ...
  wxRect rect2 =
      ((const wxRect)rect)
          .Deflate(rect.GetWidth() > 24 ? 4 : 2, rect.GetHeight() > 24 ? 4 : 2);
  ...
  if (rect2.GetWidth() > 8 && rect2.GetHeight() > 8) {
    wxRect rectMove1 =
        ((const wxRect)rect2).Deflate(rect2.GetWidth() > 24 ? 8 : 4, 0);
    wxRect rectMove2 =
        ((const wxRect)rect2).Deflate(0, rect2.GetHeight() > 24 ? 8 : 4);
...
void MenuEditor::SetMouseCursor(EditorAction tt) {
  // PERF FIX: skip if already showing the same cursor (avoids SetCursor
  // syscall)
  if (tt == m_lastCursorAction)
    return;
  m_lastCursorAction = tt;
  switch (tt) {
  case eaNONE:
  case eaSELECT_RECT:
  ...
  case eaRESIZE_TL:
  case eaRESIZE_BR:
    SetCursor(m_cursorResizeNWSE);
    break; // cached
  case eaRESIZE_TR:
  case eaRESIZE_BL:
    SetCursor(m_cursorResizeNESW);
    break; // cached
  case eaROTATE:
    SetCursor(m_cursorRotate);
    break; // cached
  }
}

// PERF FIX: timer handler - performs deferred SVG refresh (~60fps max during
// drag)
void MenuEditor::OnRefreshTimer(wxTimerEvent &WXUNUSED(event)) {
  m_refreshPending = false;
  if (m_menu && m_selectedIds.size() > 0)
    RefreshSelected();
}
...
bool MenuEditor::MoveObject(int &x, int &y) {
  ...
  if (m_doCopy) {
    ...
    for (wxArrayString::const_iterator it = m_selectedIds.begin();
      ...
      wxRect rect = obj->GetBBox();
      wxString id = m_menu->AddObject(
          obj->GetXML(DVDSTYLER_XML, m_dvd, ssmNORMAL, true), true);
      if (id.length() > 0) {
        objIds.Add(id);
    ...
    for (wxArrayString::const_iterator it = objIds.begin(); it != objIds.end();
         it++)
  ...
  for (wxArrayString::iterator it = m_selectedIds.begin();
    ...
    if (obj->IsButton()) {
      if (rect.x + x < 0)
        ...
        x = rect.x + rect.width >= maxWidth
                ? 0
                : maxWidth - rect.x - rect.width - 1;
      if (rect.y + y < 0)
        y = rect.y < 0 ? 0 : -rect.y;
      else if (rect.y + rect.height + y >= maxHeight)
        y = rect.y + rect.height >= maxHeight
                ? 0
                : maxHeight - rect.y - rect.height - 1;
    } else {
...
void MenuEditor::ShowSafeTV() {
  if (!m_doc)
    return;
  wxSVGGElement *gElem = (wxSVGGElement *)m_doc->GetElementById(wxT("safeTV"));
  if (gElem)
    gElem->GetParent()->RemoveChild(gElem);

  if (m_safeTV) {
    int safeWidth =
        m_menu->GetAspectRatio() == ar4_3 ||
                m_dvd->GetPgcArray(m_tsi, true).GetVideo().GetWidescreen() ==
                    wtNOPANSCAN
            ? 48
            : 90;                                                 // 6.7%/12.5%
    ...
    rectElem->SetHeight(m_menu->GetResolution().GetHeight() - safeHeight * 2 -
                        1);
    gElem->AppendChild(rectElem);
    rectElem = (wxSVGRectElement *)rectElem->CloneNode();
...
void MenuEditor::ShowGrid() {
  wxSVGGElement *gElem = (wxSVGGElement *)m_doc->GetElementById(wxT("grid"));
  if (gElem)
    gElem->GetParent()->RemoveChild(gElem);
  ...
  if (m_showGrid && m_grid > 0) {
    ...
    if (m_gridOverObejcts)
      ...
      m_doc->GetRootElement()->InsertBefore(
          gElem, m_doc->GetElementById(wxT("objects")));
  }
  Refresh();
}
...
void MenuEditor::OnKeyDown(wxKeyEvent &event) {
  ...
  case WXK_TAB: {
    int idx = m_selectedIds.size() > 0
                  ? m_menu->GetIndexOfObject(GetSelected()) + 1
                  : 0;
  ...
  case WXK_DOWN:
    if (m_selectedIds.size() > 0) {
      ...
      int x = key == WXK_LEFT    ? -(1 << m_grid)
              : key == WXK_RIGHT ? 1 << m_grid
                                 : 0;
      int y = key == WXK_UP     ? -(1 << m_grid)
              : key == WXK_DOWN ? 1 << m_grid
                                : 0;
      MoveObjectInt(x, y);
      RefreshSelected();
    }
...
void MenuEditor::OnObjectCopy(wxCommandEvent &WXUNUSED(event)) {
  ...
  for (wxArrayString::const_iterator it = m_selectedIds.begin();
       it != m_selectedIds.end(); it++)
    root->AppendChild(
        m_menu->GetObject(*it)->GetXML(DVDSTYLER_XML, m_dvd, ssmNORMAL, true));
...
void MenuEditor::OnObjectPaste(wxCommandEvent &WXUNUSED(event)) {
  if (wxTheClipboard->IsSupported(wxDataFormat(DATAFORMAT_MENU_OBJECT))) {
    ...
    if (m_menuPos.x != -1 && m_menuPos.y != -1) {
      for (wxArrayString::const_iterator it = objIds.begin();
           it != objIds.end(); it++) {
        MenuObject *obj = m_menu->GetObject(*it);
        if (obj) {
...
void MenuEditor::OnUpdateUIObjectPaste(wxUpdateUIEvent &event) {
  event.Enable(m_menu && (wxTheClipboard->IsSupported(wxDF_TEXT) ||
                          wxTheClipboard->IsSupported(
                              wxDataFormat(DATAFORMAT_MENU_OBJECT))));
}
...
void MenuEditor::OnUpdateUIFirstButton(wxUpdateUIEvent &event) {
  event.Enable(m_selectedIds.size() == 1 &&
               m_menu->GetObject(GetSelected())->IsButton());
  event.Check(m_selectedIds.size() == 1 &&
              m_menu->IsFirstButton(GetSelected()));
}
...
void MenuEditor::OnUpdateUIObjectForward(wxUpdateUIEvent &event) {
  event.Enable(m_selectedIds.size() == 1 &&
               !m_menu->IsLastObject(GetSelected()));
}

void MenuEditor::OnUpdateUIObjectBackward(wxUpdateUIEvent &event) {
  event.Enable(m_selectedIds.size() == 1 &&
               !m_menu->IsFirstObject(GetSelected()));
}
...
void MenuEditor::OnProperties(wxCommandEvent &event) {
  ...
  if (m_selectedIds.size() == 0) {
    ...
    if (!actionObj)
      return;
    MenuObjectPropDlg propDlg(this, m_actionObject, m_selectedIds.size() > 1,
      ...
      for (wxArrayString::const_iterator it = m_selectedIds.begin();
        ...
        for (unsigned int i = 0; i < actionObj->GetParams().size(); i++) {
          ...
          if (param->changeable && param->type == wxT("colour")) {
            ...
            obj->SetParamColour(param->name, param->highlightedColour,
                                mbsHIGHLIGHTED);
            obj->SetParamColour(param->name, param->selectedColour,
                                mbsSELECTED);
          } else if (param->type == wxT("colour")) {
            obj->SetParam(param->name, actionObj->GetParam(param->name));
            obj->SetParam(param->name,
                          actionObj->GetParam(param->name, wxT("-opacity")),
                          wxT("-opacity"));
          } else if (param->type == wxT("text")) {
            obj->SetParamFont(param->name,
                              actionObj->GetParamFont(param->name));
          } else if (param->type == wxT("shadow")) {
            obj->SetParam(param->name,
                          actionObj->GetParam(param->name, wxT("visibility")),
                          wxT("visibility"));
            obj->SetParam(param->name, actionObj->GetParam(param->name));
            obj->SetParam(param->name,
                          actionObj->GetParam(param->name, wxT("-opacity")),
                          wxT("-opacity"));
            obj->SetParamInt(param->name,
                             actionObj->GetParamInt(param->name, wxT("x")),
                             wxT("x"));
            obj->SetParamInt(param->name,
                             actionObj->GetParamInt(param->name, wxT("y")),
                             wxT("y"));
          } else if (param->type != _T("image") &&
                     param->attribute.Find(wxT('#')) == wxNOT_FOUND) {
            obj->SetParam(param->name, actionObj->GetParam(param->name));
          }
        }
        obj->UpdateSize();
...
void MenuEditor::OnAddObject(wxCommandEvent &event) {
  ...
  AddObject(m_addMenuObjects[n], (int)(m_menuPos.x / GetScaleX()),
            (int)(m_menuPos.y / GetScaleY()));
}
