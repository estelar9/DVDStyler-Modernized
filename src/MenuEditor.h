...
public:
  ...
  bool AddButton(wxString fname, int x, int y, int pgci = 0, int tsi = -2,
                 bool isMenu = false, int chapter = 0);

  wxString GetSelected() {
    return m_selectedIds.size() > 0 ? m_selectedIds[0] : wxT("");
  }
  ...
  void Update(const wxRect *rect = NULL) {
    SendChangedEvent();
    Refresh(true, rect);
  }
  void Update(const wxSVGRect *rect) {
    SendChangedEvent();
    Refresh(rect);
  }
...
private:
  ...
  bool m_doCopy;
  bool m_copied;
  // --- Performance: cursor cache (avoid disk I/O on every mouse move) ---
  wxCursor m_cursorMove;
  wxCursor m_cursorRotate;
  wxCursor m_cursorResizeNWSE;
  wxCursor m_cursorResizeNESW;
  EditorAction m_lastCursorAction;

  // --- Performance: HitTest throttle (avoid O(n) DOM scan every pixel) ---
  int m_lastHitTestX;
  int m_lastHitTestY;
  wxString m_lastHitTestResult;

  // --- Performance: render throttle timer (~60fps max during drag) ---
  ...
  void DoSelect(wxString id = wxT(""), bool refresh = false,
                bool sendEvent = false, bool ctrlDown = false);
...
extern const wxEventType EVT_COMMAND_MENUEDITOR_MENU_CHANGED;
extern const wxEventType EVT_COMMAND_MENUEDITOR_OBJECT_POINTED;

#define EVT_MENUEDITOR_MENU_CHANGED(id, fn)                                    \
  DECLARE_EVENT_TABLE_ENTRY(                                                   \
      EVT_COMMAND_MENUEDITOR_MENU_CHANGED, id, wxID_ANY,                       \
      (wxObjectEventFunction)(wxEventFunction)(wxCommandEventFunction) & fn,   \
      NULL),

#define EVT_MENUEDITOR_OBJECT_POINTED(id, fn)                                  \
  DECLARE_EVENT_TABLE_ENTRY(                                                   \
      EVT_COMMAND_MENUEDITOR_OBJECT_POINTED, id, wxID_ANY,                     \
      (wxObjectEventFunction)(wxEventFunction)(wxCommandEventFunction) & fn,   \
      NULL),
