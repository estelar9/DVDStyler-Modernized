...
public:
  ...
  int GetSelectedMenuId() {
    return GetSelectedMenu() >= 0 ? GetItem(GetSelectedMenu())->GetId() : -1;
  }

  bool AddVideo(const wxString &fname, bool createTitle = true, int tsi = -1,
                bool allowMenuDuplication = true);
...
extern const wxEventType EVT_COMMAND_MENU_SELECTED;
extern const wxEventType EVT_COMMAND_DVD_CHANGED;

#define EVT_MENU_SELECTED(id, fn)                                              \
  DECLARE_EVENT_TABLE_ENTRY(                                                   \
      EVT_COMMAND_MENU_SELECTED, id, wxID_ANY,                                 \
      (wxObjectEventFunction)(wxEventFunction)(wxCommandEventFunction) & fn,   \
      NULL),
#define EVT_DVD_CHANGED(id, fn)                                                \
  DECLARE_EVENT_TABLE_ENTRY(                                                   \
      EVT_COMMAND_DVD_CHANGED, id, wxID_ANY,                                   \
      (wxObjectEventFunction)(wxEventFunction)(wxCommandEventFunction) & fn,   \
      NULL),
