...
void EnumDevices(wxArrayString &devices, wxComboBox *deviceChoice) {
  ...
  if (!udev) {
    ...
    udev_list_entry_foreach(dev_list_entry, devs) {
      ...
      wxString devModel(udev_device_get_property_value(dev, "ID_MODEL"),
                        wxConvISO8859_1);
      devices.Add(devNode);
...
wxString GetDriveString(WCHAR *deviceId) {
  ...
  if (GetVolumeNameForVolumeMountPoint(deviceId, volume, BUFFER_SIZE)) {
    ...
    for (TCHAR c = TEXT('C'); c <= TEXT('Z'); c++) {
      drive[0] = c;
      if (GetVolumeNameForVolumeMountPoint(drive, bufd, BUFFER_SIZE) &&
          !wcscmp(bufd, volume)) {
...
wxString GetProductId(HANDLE hDevice) {
  ...
  bool status = DeviceIoControl(hDevice, IOCTL_STORAGE_QUERY_PROPERTY, &query,
                                sizeof(STORAGE_PROPERTY_QUERY), &outBuf, 512,
                                &returnedLength, NULL);
  if (!status)
    return wxT("");
  query.PropertyId = StorageDeviceProperty;
  query.QueryType = PropertyStandardQuery;
  status = DeviceIoControl(hDevice, IOCTL_STORAGE_QUERY_PROPERTY, &query,
                           sizeof(STORAGE_PROPERTY_QUERY), &outBuf, 512,
                           &returnedLength, NULL);
  if (!status)
    return wxT("");
...
wxString GetProductId(const wxString &drive) {
  ...
  if (GetVolumePathName(drive.wc_str(), volPath, MAX_PATH)) {
    ...
    if (_sntprintf(volPathWithLeadingPrefix, sizeof(volPathWithLeadingPrefix),
                   volPath[0] == _T('\\') ? _T("%s") : _T("\\\\.\\%s"),
                   volPath) < (int)sizeof(volPathWithLeadingPrefix)) {
      HANDLE hDevice = CreateFile(volPathWithLeadingPrefix, 0,
...
void EnumDevices(wxArrayString &devices, wxComboBox *deviceChoice) {
  ...
  for (char c = 'C'; c <= 'Z'; c++) {
    drive[0] = wxChar(c);
    if (GetDriveType(drive.wc_str()) == DRIVE_CDROM) {
      devices.Add(drive);
...
void EnumDevices(wxArrayString &devices, wxComboBox *deviceChoice) {
  ...
  if (d.GetFirst(&fname, wxT("dvd*"), wxDIR_FILES))
    do {
      allDevices.Add(wxT("/dev/") + fname);
    } while (d.GetNext(&fname));
  ...
  if (scanCmd.length()) {
    wxProgressDialog *pdlg = new wxProgressDialog(
        _("Scan devices"), _("Please wait..."), allDevices.GetCount() - 1,
        deviceChoice->GetParent(),
    ...
    for (unsigned int i = 0; i < allDevices.GetCount(); i++) {
      ...
      if (output.Count() > 0 && output[0].length() > 7 &&
        ...
        dev = allDevices[i] + wxT(" ") + dev.BeforeFirst(wxT(']')).Trim() +
              wxT(" ") + dev.AfterFirst(wxT('[')).BeforeFirst(wxT(']')).Trim();
