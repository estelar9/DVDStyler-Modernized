...
MenuObject::MenuObject(Menu *menu, bool vmg, wxString fileName, int x, int y,
                       wxString param)
    : MenuObjectDef(menu != NULL ? menu->GetSVG() : NULL), m_action(vmg),
      m_use(NULL), m_buttonSVG(NULL) {
  m_menu = menu;
  if (menu == NULL) {
    m_deleteSVG = true;
    ...
    m_svg->GetRootElement()->InsertChild(
        bgRect, m_svg->GetRootElement()->GetChildren());
  } else {
    m_deleteSVG = false;
    m_svg = menu->GetSVG();
  }
...
bool MenuObject::Init(wxString fileName, int x, int y, wxString param) {
  ...
  m_displayVideoFrame =
      m_button; // display video frame is default for button but not for objects
  m_previewHighlighted =
      root->GetAttribute(wxT("previewHighlighted")) == wxT("true");
  ...
  if (m_id.length()) { // load button
    ...
    wxString val;
    if (node && node->GetPropVal(wxT("keepAspectRatio"), &val) &&
...
wxString MenuObject::GenerateId(wxString prefix) {
  // PERF FIX: use a counter to avoid O(n) GetElementById scan per attempt.
  // We still verify with GetElementById to handle existing IDs in loaded docs.
  static std::map<wxString, int> s_idCounters;
  int &counter = s_idCounters[prefix];
  if (counter == 0)
    counter = 1;
  while (true) {
    wxString id = prefix + wxString::Format(wxT("%02d"), counter);
    counter++;
...
int MenuObject::GetChangebaleColourCount(bool drawButtonsOnBackground) {
  ...
  for (int i = 0; i < (int)m_params.size(); i++) {
    if (m_params[i]->isChangeable() &&
        ((m_params[i]->normalColour.IsOk() && !drawButtonsOnBackground) ||
         m_params[i]->highlightedColour.IsOk() ||
         m_params[i]->selectedColour.IsOk()))
      count++;
  }
  return count;
}
...
wxSVGSVGElement *MenuObject::AddButtonSVG(wxString id, wxSVGElement *content) {
  ...
  if (content->GetDtd() == wxSVG_SVG_ELEMENT) {
    ...
    m_buttonSVG->SetPreserveAspectRatio(
        ((wxSVGSVGElement *)content)->GetPreserveAspectRatio());
  } else if (content->GetDtd() == wxSVG_SYMBOL_ELEMENT) {
    m_buttonSVG->SetViewBox(((wxSVGSymbolElement *)content)->GetViewBox());
    m_buttonSVG->SetPreserveAspectRatio(
        ((wxSVGSymbolElement *)content)->GetPreserveAspectRatio());
  }
...
void MenuObject::AddUseElement(wxString id, int x, int y, int width, int height,
...
wxSVGSVGElement *MenuObject::GetButtonSVG() const { return m_buttonSVG; }

void MenuObject::SetScale(double scaleX, double scaleY) {
  wxSVGGElement *transElem =
      (wxSVGGElement *)GetButtonSVG()->GetElementById(TRANS_ELEM_ID);
  if (transElem == NULL) {
  ...
  // copy old values
  const wxSVGTransformList &oldTransList =
      transElem->GetTransform().GetBaseVal();
...
wxString MenuObject::GetId(bool translate) {
  ...
  return (IsButton() ? _("button") : _("object")) +
         wxString::Format(wxT(" %d"), (int)l);
}

int MenuObject::GetX() const { return m_use->GetX().GetBaseVal(); }
void MenuObject::SetX(int value) {
  m_use->SetX(value);
  UpdateTransform();
}

int MenuObject::GetY() const { return m_use->GetY().GetBaseVal(); }
void MenuObject::SetY(int value) {
  m_use->SetY(value);
  UpdateTransform();
}

int MenuObject::GetWidth() const { return m_use->GetWidth().GetBaseVal(); }
void MenuObject::SetWidth(int value) {
  m_use->SetWidth(value);
  UpdateTransform();
}

int MenuObject::GetHeight() const { return m_use->GetHeight().GetBaseVal(); }
...
void MenuObject::UpdateTransform() {
  if (m_use->GetTransform().GetBaseVal().size() > 0 &&
      m_use->GetTransform().GetBaseVal()[0].GetType() ==
          wxSVG_TRANSFORM_ROTATE) {
    wxSVGTransform &transform = m_use->GetTransform().GetBaseVal()[0];
    transform.SetRotate(transform.GetAngle(), GetX() + GetWidth() / 2,
                        GetY() + GetHeight() / 2);
  }
}
...
double MenuObject::GetAngle() const {
  ...
  return m_use->GetTransform().GetBaseVal().size() > 0 &&
                 m_use->GetTransform().GetBaseVal()[0].GetType() ==
                     wxSVG_TRANSFORM_ROTATE
             ? m_use->GetTransform().GetBaseVal()[0].GetAngle()
             : 0;
}
...
void MenuObject::SetAngle(double angle) {
  ...
  if (m_use->GetTransform().GetBaseVal().size() > 0 &&
      m_use->GetTransform().GetBaseVal()[0].GetType() ==
          wxSVG_TRANSFORM_ROTATE) {
    if (angle != 0) {
      m_use->GetTransform().GetBaseVal()[0].SetRotate(
          angle, GetX() + GetWidth() / 2, GetY() + GetHeight() / 2);
    } else {
...
wxRect MenuObject::GetResultBBox() const {
  ...
  //		delete viewportElem;
  //		bbox = wxRect(GetX() + elemBox.GetX(), GetY() + elemBox.GetY(),
  //elemBox.GetWidth(), elemBox.GetHeight());
  //	}
  if (m_use->GetTransform().GetBaseVal().size() > 0 &&
      m_use->GetTransform().GetBaseVal()[0].GetType() ==
          wxSVG_TRANSFORM_ROTATE) {
    ...
    if (angle != 0) {
      wxSVGPoint point[4] = {
          wxSVGPoint(bbox.GetX(), bbox.GetY())
              .MatrixTransform(transform.GetMatrix()),
          wxSVGPoint(bbox.GetX() + bbox.GetWidth(), bbox.GetY())
              .MatrixTransform(transform.GetMatrix()),
          wxSVGPoint(bbox.GetX(), bbox.GetY() + bbox.GetHeight())
              .MatrixTransform(transform.GetMatrix()),
          wxSVGPoint(bbox.GetX() + bbox.GetWidth(),
      ...
      return wxRect(p1.GetX(), p1.GetY(), p2.GetX() - p1.GetX(),
                    p2.GetY() - p1.GetY());
    }
  }
  return bbox;
...
wxRect MenuObject::GetFrameBBox(SubStreamMode mode, bool ignorePadding) {
  ...
  if (m_menu != NULL)
    fy *= (double)m_menu->GetFrameResolution().GetHeight() /
          m_menu->GetResolution().GetHeight();
  int width = round(bbox.GetWidth() * fx);
  int height = round(bbox.GetHeight() * fy);
  int padY = mode == ssmLETTERBOX && !ignorePadding
                 ? m_menu->GetFrameResolution().GetHeight() * 0.125
                 : 0;
  int cropX =
      mode == ssmPANSCAN ? m_menu->GetFrameResolution().GetWidth() * 0.125 : 0;
  ...
  for (unsigned int idx = 0; idx < size.elements.size(); idx++) {
    ...
    if (elem) {
      if (elem->GetDtd() == wxSVG_IMAGE_ELEMENT) {
        ...
        result +=
            width ? imgElem->GetDefaultWidth() : imgElem->GetDefaultHeight();
      } else {
        wxSVGRect bbox = wxSVGLocatable::GetElementResultBBox(elem);
        result += width ? (unsigned int)bbox.GetWidth()
                        : (unsigned int)bbox.GetHeight();
      }
    }
  }
...
void MenuObject::FixSize(int &width, int &height) {
  ...
  if (m_keepAspectRatio && m_aspectRatioElem != NULL &&
      m_aspectRatioElem->GetDefaultWidth() > 0)
    aspectRatio = ((double)m_aspectRatioElem->GetDefaultWidth()) /
                  m_aspectRatioElem->GetDefaultHeight();
  ...
  if (m_defaultSize) {
    ...
    if (m_keepAspectRatio) {
      if (aspectRatio > 0)
        ...
        height = width * CalcSize(m_defaultHeight, false) /
                 CalcSize(m_defaultWidth, true);
    } else
...
void MenuObject::UpdateSize() {
  ...
  if (IsAlignRight(m_defaultSize ? m_defaultWidth : m_minWidth) &&
      width != GetWidth()) {
...
void GetAnimationElements(wxSVGElement *child,
                          vector<wxSVGAnimateElement *> &animations) {
  while (child) {
    if (child->GetType() == wxSVGXML_ELEMENT_NODE &&
        child->GetDtd() == wxSVG_ANIMATE_ELEMENT) {
...
vector<wxSVGAnimateElement *> MenuObject::GetAnimations() {
  ...
  GetAnimationElements((wxSVGElement *)GetButtonSVG()->GetFirstChild(),
                       animations);
  return animations;
}
...
void MenuObject::Forward() {
  ...
  if (next && next->GetType() == wxSVGXML_ELEMENT_NODE &&
...
void MenuObject::Backward() {
  ...
  if (prev && prev->GetType() == wxSVGXML_ELEMENT_NODE &&
...
wxString MenuObject::GetDefaultFocusDest(NavigationButton navButton) {
  ...
  for (map<int, wxArrayString>::iterator it = btMap.begin(); it != btMap.end();
       it++)
  ...
  for (unsigned int i = 0; i < buttons.size(); i++) {
    ...
    int dist = navButton == nbLEFT || navButton == nbRIGHT
                   ? objRect.GetY() - rect.GetY()
                   : objRect.GetX() - rect.GetX();
    if (result.length() == 0) {
      result = obj->GetId();
      resultDist = dist;
...
wxSvgXmlNode *MenuObject::GetXML(DVDFileType type, DVD *dvd, SubStreamMode mode,
                                 bool withSVG) {
  wxString rootName = wxT("object");
  if (IsButton())
    rootName =
        IsAutoExecute() && type == SPUMUX_XML ? wxT("action") : wxT("button");
  ...
  case DVDSTYLER_XML:
    ...
    if (GetFocusDest(nbLEFT).length() || GetFocusDest(nbRIGHT).length() ||
      ...
      wxSvgXmlNode *directionNode =
          new wxSvgXmlNode(wxSVGXML_ELEMENT_NODE, _T("direction"));
    ...
    XmlWriteValue(rootNode, _T("filename"),
                  GetFileName().AfterLast(wxFILE_SEP_PATH));
    for (unsigned int i = 0; i < m_params.size(); i++) {
      MenuObjectParam *param = m_params[i];
      if (param->changeable) {
        wxSvgXmlNode *paramNode =
            new wxSvgXmlNode(wxSVGXML_ELEMENT_NODE, _T("parameter"));
        paramNode->AddProperty(_T("name"), param->name);
        paramNode->AddProperty(_T("normal"),
                               wxSVGPaint(param->normalColour).GetCSSText());
        paramNode->AddProperty(
            _T("highlighted"),
    ...
    if (m_displayVobId != DVD::MakeId(0, 0, 0))
      rootNode->AddProperty(wxT("vobId"),
                            wxString::Format(_T("%d"), m_displayVobId));
    break;
  case SPUMUX_XML: {
    ...
    rootNode->AddProperty(
        _T("x0"),
        wxString::Format(_T("%d"), rect.GetX() > 0 ? rect.GetX() : 0));
    rootNode->AddProperty(
        _T("y0"),
        wxString::Format(_T("%d"), rect.GetY() > 0 ? rect.GetY() : 0));
    rootNode->AddProperty(
        _T("x1"),
        wxString::Format(_T("%d"), rect.GetX() + rect.GetWidth() < mwidth
                                       ? rect.GetX() + rect.GetWidth()
                                       : mwidth));
    rootNode->AddProperty(
        _T("y1"),
        wxString::Format(_T("%d"), rect.GetY() + rect.GetHeight() < mheight
                                       ? rect.GetY() + rect.GetHeight()
                                       : mheight));
    wxString focusDest = GetFocusDest(nbLEFT).length()
                             ? GetFocusDest(nbLEFT)
                             : GetDefaultFocusDest(nbLEFT);
    if (focusDest.length())
      rootNode->AddProperty(_T("left"), focusDest);
    focusDest = GetFocusDest(nbRIGHT).length() ? GetFocusDest(nbRIGHT)
                                               : GetDefaultFocusDest(nbRIGHT);
    if (focusDest.length())
      rootNode->AddProperty(_T("right"), focusDest);
    focusDest = GetFocusDest(nbUP).length() ? GetFocusDest(nbUP)
                                            : GetDefaultFocusDest(nbUP);
    if (focusDest.length())
      rootNode->AddProperty(_T("up"), focusDest);
    focusDest = GetFocusDest(nbDOWN).length() ? GetFocusDest(nbDOWN)
                                              : GetDefaultFocusDest(nbDOWN);
    if (focusDest.length())
  ...
  case DVDAUTHOR_XML: {
    ...
    if (dvd->GetPlayAllRegister() != -1)
      action = wxString::Format(wxT("g%d=%d;"), dvd->GetPlayAllRegister(),
                                GetAction().IsPlayAll()
                                    ? (GetAction().IsPlayAllTitlesets() ? 2 : 1)
                                    : 0) +
               action;
    ...
    rootNode->AddChild(
        new wxSvgXmlNode(wxSVGXML_TEXT_NODE, wxEmptyString, action));
    break;
  }
  default:
    break;
  ...
  if (withSVG) { // used by copy & paste
    ...
    wxSvgXmlNode *defsNode =
        new wxSvgXmlNode(wxSVGXML_ELEMENT_NODE, wxT("defs"));
...
bool MenuObject::PutXML(wxSvgXmlNode *node) {
  ...
  if (fn.IsRelative()) {
    wxString btDir = wxStandardPaths::Get().GetUserDataDir() + wxFILE_SEP_PATH +
                     wxT("buttons") + wxFILE_SEP_PATH;
    if (wxFileName::FileExists(dir + fileName))
      fileName = dir + fileName;
    else if (wxFileName::FileExists(dir + wxT("deprecated") + wxFILE_SEP_PATH +
                                    fileName))
  ...
  m_autoExecute = node->GetPropVal(wxT("autoExecute"), &val) &&
                  (val == wxT("true") || val == wxT("1"));
  if (!node->GetPropVal(wxT("defSize"), &val) ||
      (val != wxT("true") && val != wxT("1")))
    m_defaultSize = false;
  ...
  if (svgNode) {
    ...
    LoadSVG(svg, svgNode);
    wxSVGSVGElement *root = svg.GetRootElement();
    if (root && XmlFindNode(root, wxT("defs")) &&
        XmlFindNode(root, wxT("use"))) {
      ...
      wxSVGUseElement *useElem =
          (wxSVGUseElement *)XmlFindNode(root, wxT("use"));
      AddUseElement(
          m_id, useElem->GetX().GetBaseVal(), useElem->GetY().GetBaseVal(),
          useElem->GetWidth().GetBaseVal(), useElem->GetHeight().GetBaseVal(),
          &useElem->GetTransform().GetBaseVal());
    }
  }
  ...
  m_keepAspectRatio = node->GetPropVal(wxT("keepAspectRatio"), &val) &&
                      (val == wxT("true") || val == wxT("1"));
  m_displayVideoFrame = !node->GetPropVal(wxT("displayVideoFrame"), &val) ||
                        (val != wxT("false") && val != wxT("0"));
  m_customVideoFrame = node->GetPropVal(wxT("customVideoFrame"), &val) &&
                       (val == wxT("true") || val == wxT("1"));
