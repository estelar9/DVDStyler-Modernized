...
public:
  wxThumb(wxImage image, wxString caption = wxEmptyString,
          wxString filename = wxEmptyString, bool noOutline = false) {
    m_filename = filename;
    m_image = image;
    SetCaption(caption);
    m_id = 0;
    m_noOutline = noOutline;
  }
...
enum { wxTHUMB_HORIZONTAL = wxHORIZONTAL, wxTHUMB_VERTICAL = wxVERTICAL };
...
public:
  ...
  bool IsCustomFile(const wxString &fname) {
    return m_customExtentions.Index(fname.AfterLast(wxT('.')).Lower()) >= 0;
  }
  ...
  inline wxThumb *GetItem(int index) {
    return index >= 0 && index < (int)m_items.GetCount() ? m_items[index]
                                                         : NULL;
  }
  ...
  inline void SetThumbnailOutline(wxThumbOutline tOutline) {
    m_tOutline = tOutline;
  }
  inline void SetThumbnailOutlineColor(const wxColour &colour) {
    m_tOutlineColour = colour;
  }
  inline void SetThumbnailOutlineSelectedColor(const wxColour &colour) {
    m_tOutlineSelectedColour = colour;
  }
  inline void SetThumbnailImageSelectedColor(const wxColour &colour) {
    m_tImageSelectedColour = colour;
  }
...
// Use plain extern (no dllimport) — these events are defined in Thumbnails.cpp,
// not imported from a DLL. The old DECLARE_EVENT_TYPE macro adds
// __declspec(dllimport) on MSW which breaks linking when built as part of the
// main executable.
extern const wxEventType EVT_COMMAND_THUMBNAILS_SEL_CHANGED;
extern const wxEventType EVT_COMMAND_THUMBNAILS_POINTED;
extern const wxEventType EVT_COMMAND_THUMBNAILS_DCLICK;
extern const wxEventType EVT_COMMAND_THUMBNAILS_CAPTION_CHANGED;
extern const wxEventType EVT_COMMAND_THUMBNAILS_THUMB_CHANGED;

#define EVT_THUMBNAILS_SEL_CHANGED(id, fn)                                     \
  DECLARE_EVENT_TABLE_ENTRY(                                                   \
      EVT_COMMAND_THUMBNAILS_SEL_CHANGED, id, wxID_ANY,                        \
      (wxObjectEventFunction)(wxEventFunction)(wxCommandEventFunction) & fn,   \
      NULL),

#define EVT_THUMBNAILS_POINTED(id, fn)                                         \
  DECLARE_EVENT_TABLE_ENTRY(                                                   \
      EVT_COMMAND_THUMBNAILS_POINTED, id, wxID_ANY,                            \
      (wxObjectEventFunction)(wxEventFunction)(wxCommandEventFunction) & fn,   \
      NULL),

#define EVT_THUMBNAILS_DCLICK(id, fn)                                          \
  DECLARE_EVENT_TABLE_ENTRY(                                                   \
      EVT_COMMAND_THUMBNAILS_DCLICK, id, wxID_ANY,                             \
      (wxObjectEventFunction)(wxEventFunction)(wxCommandEventFunction) & fn,   \
      NULL),

#define EVT_THUMBNAILS_CAPTION_CHANGED(id, fn)                                 \
  DECLARE_EVENT_TABLE_ENTRY(                                                   \
      EVT_COMMAND_THUMBNAILS_CAPTION_CHANGED, id, wxID_ANY,                    \
      (wxObjectEventFunction)(wxEventFunction)(wxCommandEventFunction) & fn,   \
      NULL),

#define EVT_THUMBNAILS_THUMB_CHANGED(id, fn)                                   \
  DECLARE_EVENT_TABLE_ENTRY(                                                   \
      EVT_COMMAND_THUMBNAILS_THUMB_CHANGED, id, wxID_ANY,                      \
      (wxObjectEventFunction)(wxEventFunction)(wxCommandEventFunction) & fn,   \
      NULL),
